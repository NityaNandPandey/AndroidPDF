...
public class CropOverlayView extends View {
    ...
    /**
     * Set the offset of the crop rect as a perentage of the image to be cropped on each side
     *
     * @param percentageMargins the margins
     */
    ...
    /**
     * Gets the percentage of content that has been cropped off from each side of the image.
     *
     * @return a RectF representing the cropped off sides as a percentage of the full image size
     */
    public RectF getCropRectPercentageMargins() {
        if (mCropPercentageRect == null)
        {
            updatePercentageRect();
        }
    ...
    private void initCropWindow(Rect bitmapRect) {
        ...
        if (mFixAspectRatio) {
            ...
            final float leftPadding = mCropPercentageRect.left * bitmapRect.width();
            final float rightPadding = mCropPercentageRect.right * bitmapRect.width();
            final float bottomPadding = mCropPercentageRect.bottom * bitmapRect.height();
            final float topPadding = mCropPercentageRect.top * bitmapRect.height();


            Edge.LEFT.setCoordinate(bitmapRect.left + leftPadding);
            Edge.TOP.setCoordinate(bitmapRect.top + topPadding);
            Edge.RIGHT.setCoordinate(bitmapRect.right - rightPadding);
    ...
    /**
     * Updates the mCropPercentageRect as a result of moving edges
     */
    private void updatePercentageRect()
    {
        if (mBitmapRect != null)
        {
            final float leftPadding = (Edge.LEFT.getCoordinate() - mBitmapRect.left ) / mBitmapRect.width();
            final float rightPadding = (mBitmapRect.right - Edge.RIGHT.getCoordinate()) / mBitmapRect.width();
            final float bottomPadding = (mBitmapRect.bottom - Edge.BOTTOM.getCoordinate()) / mBitmapRect.height();
            final float topPadding = (Edge.TOP.getCoordinate() - mBitmapRect.top ) / mBitmapRect.height();

            mCropPercentageRect = new RectF(leftPadding, topPadding, rightPadding, bottomPadding);
        }
    }
}
